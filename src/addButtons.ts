  ...
  projectAddButton.addEventListener("click", () => {
    if (projectsSetup.projects[0]) {
      let lastProjectNumber = parseInt(
        projectsSetup.projects[projectsSetup.projects.length - 1].name.replace(
          /^\D+/g,
          ""
        )
      );
      if (
        projectsSetup.projects[projectsSetup.projects.length - 1].name ===
...
export function addTask() {
  taskAddButton.addEventListener("click", () => {
    if (projectsSetup.projects[0]) {
      ...
      projectsSetup.projects[projectIndex].addTask(
        `Task ${numOfTasks}`,
        newDate,
        "Description",
        false,
