...
function addProject() {
  projectAddButton.addEventListener("click", () => {
    if (_1.projectsSetup.projects[0]) {
      let lastProjectNumber = parseInt(
        _1.projectsSetup.projects[
          _1.projectsSetup.projects.length - 1
        ].name.replace(/^\D+/g, "")
      );
      if (
        _1.projectsSetup.projects[_1.projectsSetup.projects.length - 1].name ===
        `Project ${lastProjectNumber}`
      ) {
...
function addTask() {
  exports.taskAddButton.addEventListener("click", () => {
    if (_1.projectsSetup.projects[0]) {
      let numOfTasks = 1;
      _1.projectsSetup.projects[uiUpdater_1.projectIndex].tasks.forEach(
        (task) => {
          numOfTasks += 1;
        }
      );
      _1.projectsSetup.projects[uiUpdater_1.projectIndex].addTask(
        `Task ${numOfTasks}`,
        newDate,
        "Description",
        false,
      ...
      (0, uiUpdater_1.loopTasks)(
        _1.projectsSetup.projects[uiUpdater_1.projectIndex]
      );
    } else {
      return;
    }
